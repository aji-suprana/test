...
const saveJenisRekening = async (data) => {
  try {
    const { sub_kelompok_id } = data;

    const checkSubKelompok = await subKelompokRepository.findOneSubKelompok(
      sub_kelompok_id
    );
...
const findAllJenisRekenings = async (params, pagination) => {
  try {
...
const findOneJenisRekening = async (id) => {
  try {
...
const updateJenisRekening = async (id, data) => {
  try {
    const { nama, nomor_jenis_rekening } = data;
    const jenisRekening = await findOneJenisRekening(id);
...
const destroyJenisRekening = async (id) => {
  try {
    const jenisRekening = await findOneJenisRekening(id);
